...
async def execute_run(run_id: int):
    with Session(engine) as session:
        ...
        try:
            bb_client = BlueBubblesClient(settings.bluebubbles_url, settings.bluebubbles_password)
            chat = find_chat_by_number(bb_client, run.from_number)
            if not chat:
                run.status = RunStatus.failed
                session.add(run)
                session.commit()
                print(f"Failed to find BlueBubbles chat with Linq number {run.from_number}")
                return
        ...
        for index, step in enumerate(scenario.steps):
            ...
            try:
                ...
                step_result.failure_reasons = [f"Failed to send message via BlueBubbles: {e}"]
                all_passed = False
                session.add(step_result)
                session.commit()
                continue
            ...
            while time.time() - start_time < timeout_seconds:
                try:
                    messages = bb_client.get_chat_messages(chat_guid, limit=10)
                    for msg in reversed(messages):
                        ...
                        if not msg.get("isFromMe"):
                            ...
                            valid_string_reactions = ['love', 'like', 'dislike', 'laugh', 'emphasize', 'question']
                            if assoc_type in valid_string_reactions:
            ...
            if response_found:
                ...
                for assertion in step.assert_:
                    ...
                    if result.passed:
                        ...
                        failure_reasons.append(f"[{assertion.type.value}] {result.reason}")
                ...
                step_result.failure_reasons = [f"Timeout waiting for reply via BlueBubbles after {scenario.timeout_ms}ms"]
                all_passed = False
